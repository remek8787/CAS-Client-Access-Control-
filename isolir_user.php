...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['username'], $_POST['router'])) {
    ...
    $res = bridge_post('/ppp/isolate', array(
        'router_id' => $routerId,
        'username' => $username,
    ...
    if (!empty($res['ok'])) {
        ...
        cas_redirect_back('isolir', 'Berhasil mengisolir ' . $username, 'ppp_users.php');
    }

    die("❌ Gagal isolir via bridge: " . h(isset($res['error']) ? $res['error'] : 'unknown'));
}
